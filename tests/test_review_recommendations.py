...
class TestMatchingBoundaries:
    def test_fuzzy_candidates_are_excluded_when_exact_match_exists(self) -> None:
        """When an exact alias match exists, fuzzy candidates are not generated."""
        from sea_mile.matching import generate_source_aware_candidates
        ...
        ids = set(candidates["registry_id"])
        # Exact match present, fuzzy "pireus" skipped
        assert "WPI:1" in ids
        assert "WPI:2" not in ids

    def test_custom_policy_changes_fuzzy_cutoff(self) -> None:
        """A stricter policy can exclude fuzzy candidates."""
        from sea_mile.matching import MatchPolicy, generate_source_aware_candidates
            ...
            [
                {
                    "query_id": "Q1",
                    "country_code": "GR",
                    "destination_key": "piraeus harbour",
                }
            ]
        )
        aliases = pd.DataFrame(
            [
                {
                    ...
                    "provider": "NGA_WPI",
                    "country_code": "GR",
                    "alias_key": "piraeus port",
                },
            ]
        )
        # Default policy (cutoff 80) — "piraeus port" vs "piraeus harbour"
        default = generate_source_aware_candidates(queries, aliases)
        assert len(default) >= 1

        # Very strict policy (cutoff 99) — weak_fuzzy still applies
        strict = MatchPolicy(fuzzy_score_cutoff=99.0, weak_fuzzy_cutoff=99.0)
        strict_candidates = generate_source_aware_candidates(
            queries, aliases, policy=strict
        )
        # With both cutoffs at 99, nothing passes
        assert strict_candidates.empty
